...
| `hook_get_modal_listener_definitions` | `block-modal-events` | **Optional, not a declared dependency** — subscribed purely by function-name convention (`Wrapper_Hooks.rebuild_hook_subs_cache` matches any block module defining the function, regardless of `block_dependencies`). Returns one inert, always-pass-through `Modal_Listener_Definition` while the textbox demo's mouse-capture toggle is on, `[]` otherwise (including when `block_modal_events` isn't active — the import is local to the function and guarded by `try/except ImportError`). Exists only to keep the modal router alive so `USER_INPUT_CAPTURE` populates for the **At Mouse** spawn option. |
...
| **Text boxes** | `textbox_lines` (UIList), `textbox_spawn_point`, `textbox_x_offset`/`textbox_y_offset`, `textbox_bg_enabled` + colors | One `Textbox_Demo_Shader` line per row of the `textbox_lines` UIList (add/remove via the toolbar above it; select a row to edit its `text`, `font_size`, `alignment`, `max_char_count` wrap width, `padding`, and `text_color` in the collapsible **Text Line Details** sub-subpanel — the same UIList/detail-section pattern as the Shaders list, but pure BL data with no RTC-backed row). `spawn_point` is a manually-expanded radio (TOP_LEFT / TOP_RIGHT / BOTTOM_LEFT / BOTTOM_RIGHT / **MOUSE**) so the MOUSE entry alone can be disabled. `textbox_x_offset` / `textbox_y_offset` shift the whole group of boxes (in px) **away from** the chosen anchor corner — the offset sign is corner-relative, so "positive" always reads as "push toward center" regardless of which corner is selected. The shared background quad is optional (`textbox_bg_enabled` + `textbox_bg_color_top`/`bottom`). The **At Mouse** option positions boxes at the captured mouse; it requires a live `block_modal_event` instance. Since this block does not depend on `block_modal_events`, a **Start/Stop Mouse Capture** button (clickable only while that block is active, checked via `USER_INPUT_CAPTURE` RTC-key presence, never an import) toggles a lightweight, self-owned, always-pass-through listener via `hook_get_modal_listener_definitions` — subscribed by function-name convention only, purely to keep `USER_INPUT_CAPTURE` populated. |
