...
| **Dashed polyline** | `linedash_thickness`, `linedash_dash_width`, `linedash_dash_ratio`, `linedash_color`, `linedash_color2` + unique attrs `phase` (0–1, hard-capped) and cluster `count` + `is_animating` | A `Polyline_Dash_Shader` port of the legacy dashed-line shader with true Metal-safe thickness. `count` draws that many extra **disjointed, radially-symmetric ring clusters** stacked in Z above the base square — proving the polyline shader handles disjointed line clusters in one batch. Visibility is governed solely by the demo's `show_shader` (eye) toggle. Animatable. |
| **Text boxes** | `textbox_lines` (UIList), `textbox_draw_mode`, 2D: `textbox_spawn_point`+offsets. 3D: `textbox_anchor_x`/`y`, `textbox_3d_x`/`y`/`z`, `textbox_bg_enabled` + colors | One `Textbox_Demo_Shader` line per row of the `textbox_lines` UIList (add/remove via the toolbar above it; select a row to edit its `text`, `font_size`+`max_char_count` wrap width, `alignment`+`text_color`, per-line `padding_mode` (Simple / Horizontal-Vertical / Top-Right-Bottom-Left) with its value sliders, and an optional soft-shadow **Outline** (`outline_color` + `outline_spread` — Blender's `blf.shadow()` only accepts blur levels 0/3/5 — + `outline_offset_x`/`y`, zero offset reading as a symmetric outline rather than a drop-shadow) in an always-expanded **Text Line Details** box below the list — same UIList/detail-section pattern as the Shaders list, but pure BL data with no RTC-backed row. `ensure_default_textbox_lines()` seeds a few example rows exactly once (guarded by `textbox_lines_seeded`, so deleting them doesn't bring them back). Lines always stack vertically, top to bottom. The shared background quad is optional (`textbox_bg_enabled` + `textbox_bg_color_top`/`bottom`), positioned however the active draw mode resolves the box. `textbox_draw_mode` (**2D**/**3D**) picks the anchor system: **2D** — `spawn_point` is a radio (TOP_LEFT / TOP_RIGHT / BOTTOM_LEFT / BOTTOM_RIGHT / **CENTER**); `textbox_x_offset`/`textbox_y_offset` shift the box away from the chosen anchor, corner-relative (CENTER has no corner, so its offset is absolute). **3D** — the box anchors to a literal world-space point (`textbox_3d_x`/`y`/`z`, plain sliders shown only in this mode) drawn as a small yellow builtin-`POINT_UNIFORM_COLOR` marker (`_EXAMPLE_TEXTBOX_POINT_UID`, declared only while 3D mode is active); `textbox_anchor_x`/`textbox_anchor_y` (Start/Middle/End, independent per axis) place the point on a 3x3 grid over the box's bounding rectangle. Because the projected screen position of a 3D point changes every frame as the camera moves — unlike the 2D anchors, which only depend on region size — 3D mode resolves box position in `_shader_draw()` every frame (`_reposition_for_3d_frame()`, via `bpy_extras.view3d_utils.location_3d_to_region_2d()`) instead of caching it in `_shader_update_batch()`; the expensive word-wrap/measurement pass is still cached, only the cheap position math + background-quad batch rebuild rerun per frame. The text itself is always drawn via BLF in POST_PIXEL screen space regardless of mode, so it's inherently billboarded (always faces the viewer) with no separate orientation math needed. |
| **Stripe holdout** | `stripe_angle`, `stripe_width`, `stripe_color1`, `stripe_color2` + unique `phase` | Draws a unit cube of TRIs at world-space points but computes an **alternating-stripe pattern purely from window-space pixels** (`gl_FragCoord.xy`) in the fragment shader. The 2D pattern is therefore screen-locked — orbiting the camera makes the static stripes *slide across* the geometry, the intended "glitchy" holdout effect. `stripe_angle` (degrees), `stripe_width` (shared band width in px), the two stripe colors, and a `phase` slider (band scroll offset) are all controllable from the panel. Visibility is governed solely by the demo's `show_shader` (eye) toggle. Animatable: its infinite-loop animation scrolls **only** the `phase`, making the banded holdout crawl along the stripe direction. |
